...
tf.compat.v1.disable_eager_execution()  # Legacy with 1.0
...
# Input Layer
with tf.compat.v1.variable_scope('input'): # Scope only to keep things organized
    X = tf.compat.v1.placeholder(tf.float32, shape = (None, number_of_inputs))

# Layer 1
with tf.compat.v1.variable_scope('layer_1'):
    weights = tf.compat.v1.get_variable(name="weights1", shape=[number_of_inputs, layer_1_nodes], initializer=tf.keras.initializers.GlorotNormal() )
...
with tf.compat.v1.variable_scope('layer_2'):
    weights = tf.compat.v1.get_variable(name="weights2", shape=[layer_1_nodes, layer_2_nodes], initializer=tf.keras.initializers.GlorotNormal() )
...
with tf.compat.v1.variable_scope('layer_3'):
    weights = tf.compat.v1.get_variable(name="weights3", shape=[layer_2_nodes, layer_3_nodes], initializer=tf.keras.initializers.GlorotNormal() )
...
with tf.compat.v1.variable_scope('output'):
    weights = tf.compat.v1.get_variable(name="weights4", shape=[layer_3_nodes, number_of_outputs], initializer=tf.keras.initializers.GlorotNormal() )
...
with tf.compat.v1.Session() as session:
    ...
    for epoch in range(training_epochs):
        ...
        if epoch % 5 == 0:
            ...
            print(epoch, "- Training set cost:", training_cost, "Test set cost:", testing_cost)
    ...
    print("Training is complete! Model saved to {}".format(save_path) )
    ...
    print("FINAL COST Training set:", training_cost)
    print("FINAL COST Test set:", testing_cost)
